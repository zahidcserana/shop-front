import { PayoutModel } from './../models/sale.model';
...
@Component({
  selector: 'app-sale-due',
  templateUrl: './sale-due.component.html',
  styleUrls: ['./sale-due.component.css']
})
export class SaleDueComponent implements OnInit {
  currency = '৳';
  dataList: SaleModel[] = [];
  pagi: Pagi = new Pagi();
  filter: string;
  ...
  ngOnInit() {
    this.saleDueList(this.pagi.page, this.pagi.limit, this.filter);
  }
  saleDueList(p, l, q) {
    this.saleService.saleDueList(p, l, q)
      .subscribe(res => {
  ...
  reloadTable(e) {
    this.saleDueList(e.page, e.limit, e.filter);
  }
  trackList(index, pro) {
  ...
  getSaleDetails(saleId) {
    this.homeService.saleDetails(saleId)
      .subscribe((data) => {
        ...
        // this.payout.status = data.status;
      });
  }
  payoutAmount() {
    this.saleService.payoutAmount(this.payout)
      .then(
        res => {
        Swal.fire({
          position: "center",
        ...
        this.getSaleDetails(this.payout.sale_id);
        this.saleDueList(this.pagi.page, this.pagi.limit, this.filter);
      });
  }
  itemReturn(item) {
  ...
  filterList(e) {
    this.filter = e;
    this.saleDueList(1, 20, this.filter);
  }
  /* Filter end */
}
